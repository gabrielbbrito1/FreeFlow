...
Após criar e ativar o ambiente virtual, baixe as dependências que estão em `requirements.txt` na raiz do projeto:
...
Crie um arquivo `.env` na **raiz do projeto** ou dentro da pasta `backend/` com o conteúdo abaixo. O Django carrega com `python-dotenv`, então o caminho deve bater com onde está o `manage.py` (ou ajuste o `load_dotenv()` no `settings.py`).
...
Partindo da raiz do projeto e fora do ambiente virtual:
